import { Alert, Image, ImageProps, ScrollView, StyleSheet, Text, TouchableOpacity, View } from 'react-native'
...
const DetailScreen = ({ navigation, route }: any) => {
    ...
    useEffect(() => {
        const getAllCoffee = async () => {
            try {
                setLoading(true)
                const response = await fetch(`http://localhost:9000/api/v1/coffee/${id}`, {
                    method: "GET"
                })
                const data = await response.json();
                console.log("data in detailscreen", data);
